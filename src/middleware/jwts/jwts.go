...
type (
	...
	errorHandler func(iris_context.Context, string)
	...
	TokenExtractor func(iris_context.Context) (string, error)
...
func Serve(ctx iris_context.Context) bool {
	ConfigJWT()
...
// 解析token的信息为当前用户
func ParseToken(ctx iris_context.Context) (*model.User, bool) {
...
func FromAuthHeader(ctx iris_context.Context) (string, error) {
...
func FromParameter(param string) TokenExtractor {
	return func(ctx iris_context.Context) (string, error) {
...
func (m *Jwts) Get(ctx iris_context.Context) *jwt.Token {
	return ctx.Values().Get(m.Config.ContextKey).(*jwt.Token)
}

// CheckJWT the main functionality, checks for token
func (m *Jwts) CheckJWT(ctx iris_context.Context) error {
...
func LoginAsGuest(ctx iris_context.Context) {
	...
	token, err = GenerateToken(mUser)
	r := ctx.Request()
	r.Header.Set("Authorization", "bearer "+token) // for this time to pass
	ctx.Header("Access-Control-Expose-Headers", "Authorization")
	ctx.Header("Authorization", "bearer "+token) // for response for next time
	ctx.Header("Access-Control-Request-Headers", "x-requested-with")
	ctx.Header("Access-Control-Request-Method", "GET,POST")
	ctx.Header("Access-Control-Allow-Methods", "*")
	ctx.Header("Access-Control-Allow-Headers", "*")
	golog.Infof("用户[%s], 登录生成token [%s]", mUser.Username, token)
	if err != nil {
		ctx.Application().Logger().Errorf("用户[%s]登录，生成token出错。%s", "Guest", err.Error())
...
func ConfigJWT() {
	...
	c := Config{
		...
		//验证未通过错误处理方式
		ErrorHandler: func(ctx iris_context.Context, errMsg string) {
